...
LINEEND=" \\"
COUNT=`wc -l ../../$VENDOR/$DEVICE/proprietary-blobs.txt | awk {'print $1'}`
DISM=`egrep -c '(^#|^$)' ../../$VENDOR/$DEVICE/proprietary-blobs.txt`
COUNT=`expr $COUNT - $DISM`
for FILE in `egrep -v '(^#|^$)' ../../$VENDOR/$DEVICE/proprietary-blobs.txt`; do
  COUNT=`expr $COUNT - 1`
  if [ $COUNT = "0" ]; then
    LINEEND=""
  fi
